...
public class BlockPickup extends BlockBase{
	...
	public void onEntityCollidedWithBlock(World worldIn, BlockPos pos, IBlockState state, Entity entityIn) {
		if(entityIn instanceof EntityPlayer) {
			...
			if(particleNum<1) particleNum++;
			for(int i = 0;i<particleNum;i++) {
				worldIn.spawnParticle(type.getPickupTypeParticle(), pos.getX()+rand.nextFloat(), pos.getY(), pos.getZ()+rand.nextFloat(), rand.nextGaussian() * 0.02D, rand.nextGaussian() * 0.02D, rand.nextGaussian() * 0.02D);
			}

...
public class BlockCrate extends BlockBase{
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn,
		...
		if(playerIn.getHeldItemMainhand().getItem() == ItemList.mannco_key) {
			...
			worldIn.playSound(playerIn, pos, new SoundEvent(new ResourceLocation("minecraft", "entity.generic.explode")), SoundCategory.BLOCKS, 1F, 1F);
			worldIn.spawnParticle(EnumParticleTypes.EXPLOSION_LARGE, pos.getX()+.5F, pos.getY()+.5F, pos.getZ()+.5F, motion, motion, motion);
			if(!worldIn.isRemote) {
				worldIn.spawnEntity(new EntityItem(worldIn, pos.getX()+.5F, pos.getY()+.5F, pos.getZ()+.5F, new ItemStack(list.get(rand.nextInt(list.size())))));
